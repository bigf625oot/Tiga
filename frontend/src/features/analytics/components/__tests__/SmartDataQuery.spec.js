...
describe('SmartDataQuery.vue', () => {
  ...
  it('sendMessage handles streaming response correctly', async () => {
    ...
    // Mock fetch response with SSE-like JSON lines
    const encoder = new TextEncoder();
    const stream = new ReadableStream({
        start(controller) {
            controller.enqueue(encoder.encode(JSON.stringify({ step: 1, type: 'data', content: 'Hello ' }) + '\n'));
            controller.enqueue(encoder.encode(JSON.stringify({ step: 2, type: 'data', content: 'World' }) + '\n'));
            controller.close();
        }
    });
    ...
    expect(messages[0]).toMatchObject({ role: 'user', content: 'Test Question' });
