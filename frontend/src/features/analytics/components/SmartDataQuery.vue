                    ...
                    <div v-if="msg.role === 'assistant'" class="flex flex-col">
                        ...
                        <div class="p-4">
                            ...
                            <div class="min-h-[60px]">
                                <!-- Table View (Markdown) -->
                                <div 
                                    v-show="getMsgViewMode(msg._id) === 'table'" 
                                    class="markdown-body" 
...
const templates = [
    { name: '📊 统计表数据量 (柱状图)', content: '统计数据库中各表的数据量，并绘制柱状图' },
    { name: '📈 销售趋势分析 (折线图)', content: '查询 orders 表中最近 7 天的订单数量趋势，绘制折线图' },
    { name: '🥧 用户分布统计 (饼图)', content: '统计 users 表中不同地区的用户分布，绘制饼图' }
];
...
const renderMarkdown = (text) => {
    try {
        ...
        // [Extract] Extract references before stripping them
        let references = {};
        const refHeaderPattern = /\n+\s*(?:#+\s*)?(?:\*\*)?(References|Sources|参考来源|引用|引用文献|Reference Document List)(:|\：)?(\*\*)?\s*(\n+|$)/i;
        const match = inputText.match(refHeaderPattern);
        
        if (match) {
            const splitIndex = match.index;
            const refContent = inputText.substring(splitIndex + match[0].length);
            inputText = inputText.substring(0, splitIndex); // Keep only main text
            
            // Parse references from the stripped content
            const refLines = refContent.split('\n');
            refLines.forEach(line => {
                const lineTrim = line.trim();
                if (!lineTrim) return;
                // Matches: [1] text, 1. text, 1 text
                const refMatch = lineTrim.match(/^\[?(\d+)\]?[\.:\s]\s*(.*)$/);
                if (refMatch) {
        ...
        // [Enhance] Convert [n] to interactive elements with tooltips
        html = html.replace(/\[(\d+)\]/g, (match, id) => {
            const title = references[id] ? references[id].replace(/"/g, '&quot;') : '引用来源 ' + id;
            const content = references[id] ? encodeURIComponent(references[id]) : '';
            return `<span class="citation-index" title="${title}" data-idx="${id}" data-content="${content}">[${id}]</span>`;
        });
...
const handleContentClick = (e) => {
    const target = e.target.closest('.citation-index');
    if (target) {
        e.stopPropagation();
        const id = target.dataset.idx;
        const content = target.dataset.content ? decodeURIComponent(target.dataset.content) : null;
...
.markdown-body :deep(.citation-index) {
    color: #4f46e5;
    cursor: pointer;
    font-weight: 500;
    padding: 0 2px;
...
.markdown-body :deep(.citation-index:hover) {
