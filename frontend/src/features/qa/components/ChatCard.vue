<template>
  ...
  >
    ...
    >
      ...
      <!-- Bubble -->
      <div 
        class="relative px-4 py-3 text-sm leading-relaxed transition-all duration-200 shadow-sm"
        :class="bubbleClasses"
      >
        ...
        <div v-else class="agent-content flex flex-col gap-3">
            ...
            <div v-if="parsed.think" class="w-full">
                <details class="bg-indigo-50/30 rounded-lg border border-indigo-100/50 overflow-hidden group transition-all duration-300" :open="parsed.think.isPartial">
                    <summary class="px-3 py-2 text-xs font-medium text-indigo-500 cursor-pointer flex items-center gap-2 select-none outline-none hover:bg-indigo-50/50 transition-colors">
                    ...
                    <div class="px-3 py-2 text-xs text-slate-600 border-t border-indigo-100/30 bg-white/50 leading-relaxed font-mono" v-html="parsed.think.html"></div>
                </details>
            </div>
            ...
            <div v-if="chartOption" class="w-full bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                 <div class="px-3 py-2 border-b border-slate-50 bg-slate-50/50 flex items-center justify-between">
            ...
            <div v-if="parsed.text || parsed.html || parsed.sql" class="w-full bg-white rounded-lg border border-slate-100 shadow-sm p-3 group/summary">
                ...
                <div v-if="chartOption" class="mb-2 pb-2 border-b border-slate-50 flex items-center gap-2">
                     ...
                     <span class="text-xs font-bold text-slate-600">数据详情</span>
                </div>
                ...
                <!-- Embedded Resources -->
                <div v-if="parsed.resources.length > 0" class="mt-3 flex flex-col gap-2">
                    <GenericResourceCard 
                ...
                <div v-if="parsed.sql" class="mt-3 pt-2 border-t border-slate-50">
                    <details class="group/sql">
                        <summary class="flex items-center gap-2 text-[10px] text-slate-400 cursor-pointer hover:text-blue-500 transition-colors select-none w-fit">
                        ...
                        <div class="mt-2 relative rounded bg-slate-50 border border-slate-100">
                             <div class="absolute top-1 right-1">
                                <button @click.stop="copyText(parsed.sql)" class="p-1 text-slate-400 hover:text-blue-500 transition-colors" title="复制 SQL">
                                    <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <!-- 4. References (Footer) -->
            <div v-if="hasReferences" class="mt-1 pt-2 border-t border-slate-100/50">
                <div class="flex items-center gap-2 mb-2">
                    <svg class="w-3 h-3 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
                    </svg>
                    <span class="text-xs font-semibold text-slate-500">参考来源</span>
                </div>
                <div class="flex flex-wrap gap-2">
                    <div 
                        v-for="(ref, idx) in combinedSources" 
                        :key="idx"
                        class="group/ref flex items-center gap-1.5 px-2 py-1 bg-slate-50 border border-slate-200 rounded text-[11px] text-slate-600 cursor-pointer hover:bg-white hover:border-indigo-200 hover:shadow-sm hover:text-indigo-600 transition-all duration-200 max-w-[200px]"
                        @click="$emit('locate-node', ref)"
                        :title="ref.title || 'Unknown Source'"
                    >
                        <span class="font-mono text-slate-400 group-hover/ref:text-indigo-400 text-[9px]">{{ Number(idx) + 1 }}</span>
        ...
        <!-- Actions (Footer) -->
        <div v-if="!isUser" class="mt-2 pt-2 flex items-center gap-3 text-slate-400 border-t border-slate-100/50 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
...
const bubbleClasses = computed(() => {
  if (props.isUser) {
    return 'bg-indigo-600 text-white rounded-2xl rounded-tr-sm';
  } else {
    return 'bg-white text-slate-700 rounded-2xl rounded-tl-sm border border-slate-200';
  }
});
