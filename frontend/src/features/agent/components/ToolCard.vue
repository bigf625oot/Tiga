<template>
  <div 
    class="group bg-white border border-slate-200 rounded-lg hover:shadow-xl hover:border-blue-200 transition-all duration-300 flex flex-col h-[260px] relative overflow-hidden"
  >
    <div class="p-6 flex flex-col h-full">
      ...
      <div class="flex gap-4 m-4">
        ...
        <div 
          v-if="item.iconUrl" 
          class="relative flex-shrink-0 w-12 h-12 bg-muted/50 rounded-lg p-0.5 group-hover:scale-105 transition-transform duration-300 flex items-center justify-center"
        >
        ...
        <div 
          v-else 
          class="relative flex-shrink-0 w-12 h-12 flex items-center justify-center text-xl text-muted-foreground bg-muted/50 rounded-lg border border-border shadow-sm group-hover:scale-105 transition-transform duration-300"
        >
        ...
        <div class="flex-1 min-w-0 flex flex-col justify-between py-0.5">
          <div class="flex justify-between items-start">
            <h3 class="font-semibold text-slate-800 text-base leading-tight truncate pr-2 group-hover:text-primary transition-colors" :title="item.name">
              {{ item.name }}
            </h3>
            ...
            <div v-if="!item.is_official" class="relative flex-shrink-0">
              <button 
                ...
                :class="{'opacity-100 bg-muted text-slate-600': activeMenuId === item.id}"
              >
          ...
          <div class="flex flex-wrap items-center gap-2 mt-1">
            <span class="px-2 py-0.5 rounded-md text-[10px] font-semibold border tracking-wide uppercase" 
            ...
            <span v-if="item.is_official" class="px-2 py-0.5 rounded-md text-[10px] font-semibold bg-primary/10 text-primary border border-blue-100 flex items-center gap-1">
      ...
      <!-- Description -->
      <p class="text-xs text-muted-foreground leading-relaxed line-clamp-3 mb-4 flex-1 h-[4.5em] group-hover:text-slate-600 transition-colors" :title="item.description">
      ...
      <div class="mt-auto pt-4 border-t border-slate-50 flex flex-col gap-4">
        <div class="flex items-center justify-between text-[11px] text-muted-foreground">
          <div class="flex items-center gap-1.5">
            <div class="w-4 h-4 rounded-full bg-muted flex items-center justify-center text-[8px] font-semibold text-muted-foreground">
          ...
          <div class="flex items-center gap-4">
            ...
            <span class="bg-muted/50 px-1.5 py-0.5 rounded text-muted-foreground font-mono">v{{ item.version }}</span>
          </div>
        </div>
        ...
        <button 
          ...
          :class="item.installed || (item.is_active === false)
            ...
            : 'bg-white border-slate-200 text-slate-700 hover:text-primary hover:border-blue-200 hover:shadow-md hover:bg-primary/10/30 active:scale-[0.98]'"
