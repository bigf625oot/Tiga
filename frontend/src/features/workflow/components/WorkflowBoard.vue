<template>
  <div class="h-full flex flex-col bg-muted/50">
     <!-- Stats Header -->
     <div class="bg-white p-4 border-b border-slate-200 flex items-center gap-6 shadow-sm z-10">
        <div class="flex flex-col">
            <span class="text-xs text-muted-foreground">任务进度</span>
            <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-semibold text-indigo-600">{{ store.progress }}%</span>
            </div>
        </div>
        ...
        <div class="flex gap-4">
             ...
             <div class="flex flex-col">
                 ...
                 <span class="text-sm font-medium" :class="store.isRunning ? 'text-primary' : 'text-slate-600'">
        ...
        <div class="flex gap-2">
             <button v-if="store.isRunning" @click="store.stopWorkflow" class="p-4 py-1.5 bg-red-50 text-red-600 border border-red-100 rounded hover:bg-red-100 text-sm">
                 停止执行
             </button>
        </div>
     </div>
     ...
     <div class="flex-1 flex overflow-hidden">
        ...
        <div class="w-1/2 p-4 overflow-y-auto border-r border-slate-200 bg-white">
            <h3 class="text-sm font-semibold text-slate-700 m-4 flex items-center gap-2">
