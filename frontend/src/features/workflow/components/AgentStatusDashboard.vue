<template>
  <div class="agent-status-dashboard bg-white rounded-lg shadow-sm border border-slate-200 p-6 flex flex-col gap-6">
    ...
    <div class="flex items-start justify-between">
      <div class="flex items-center gap-4">
        <div class="relative w-14 h-14 shrink-0">
          ...
          <div class="relative w-full h-full rounded-full bg-muted border-2 border-white shadow-md overflow-hidden flex items-center justify-center">
        ...
        <div class="flex flex-col">
          <h3 class="font-semibold text-slate-800 text-lg leading-tight">{{ agentName }}</h3>
          <div class="flex items-center gap-2 text-xs text-muted-foreground font-mono mt-1">
            <span class="bg-muted px-1.5 py-0.5 rounded text-slate-600 border border-slate-200">{{ sessionId.slice(0, 8) }}</span>
      ...
      <div class="flex flex-col items-end gap-1">
        <div class="p-4 py-1 rounded-full bg-indigo-50 text-indigo-700 text-xs font-semibold border border-indigo-100 flex items-center gap-1.5 shadow-sm">
    ...
    <div class="flex flex-col gap-2">
      <div class="flex justify-between items-end">
        <span class="text-xs font-semibold text-slate-700 uppercase tracking-wider">{{ t('agentStatus.progress') }}</span>
        <span class="text-sm font-semibold text-indigo-600">{{ progress }}%</span>
      </div>
      <div class="w-full bg-muted rounded-full h-3 overflow-hidden shadow-inner border border-slate-200/60">
        <div 
        ...
        >
          ...
          <div 
            ...
            :class="[
              ...
              'bg-white border-border text-muted-foreground grayscale'
            ]"
          >
            <div 
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-border">
       ...
       <div class="bg-muted/50 rounded-lg p-2.5 border border-border flex flex-col gap-1.5">
         ...
         <div class="flex items-center justify-between">
            <span class="text-sm font-mono font-semibold text-slate-700">{{ cpuUsage || 0 }}%</span>
       ...
       <div class="bg-muted/50 rounded-lg p-2.5 border border-border flex flex-col gap-1.5">
         ...
         <div class="flex items-center justify-between">
            <span class="text-sm font-mono font-semibold text-slate-700">{{ memoryUsage || 0 }}MB</span>
            <div class="w-12 h-1 bg-slate-200 rounded-full overflow-hidden">
       ...
       <div class="bg-muted/50 rounded-lg p-2.5 border border-border flex flex-col gap-1.5">
         ...
         <div class="flex items-center gap-2">
            <span class="text-sm font-mono font-semibold text-slate-700">↓{{ networkUsage || 0 }}KB/s</span>
         </div>
       </div>
