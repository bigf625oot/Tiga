<template>
  <div class="agent-status-dashboard bg-card rounded-lg shadow-sm border border-border p-6 flex flex-col gap-6">
    ...
    <div class="flex items-start justify-between">
      <div class="flex items-center gap-4">
        ...
        <div class="flex flex-col">
          <h3 class="font-semibold text-foreground text-lg leading-tight">{{ agentName }}</h3>
          <div class="flex items-center gap-2 text-xs text-muted-foreground font-mono mt-1">
            <span class="bg-muted px-1.5 py-0.5 rounded text-muted-foreground border border-border">{{ sessionId.slice(0, 8) }}</span>
      ...
      <div class="flex flex-col items-end gap-1">
        <div class="p-4 py-1 rounded-full bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-xs font-semibold border border-indigo-500/20 flex items-center gap-1.5 shadow-sm">
    ...
    <div class="flex flex-col gap-2">
      <div class="flex justify-between items-end">
        <span class="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{{ t('agentStatus.progress') }}</span>
        <span class="text-sm font-semibold text-indigo-600 dark:text-indigo-400">{{ progress }}%</span>
      </div>
      <div class="w-full bg-muted rounded-full h-3 overflow-hidden shadow-inner border border-border/60">
        <div 
    ...
    <div class="w-full overflow-x-auto pb-2 -mb-2 scrollbar-thin">
      <div class="flex items-center min-w-max gap-1">
        ...
        >
          <!-- Step Item -->
          <div 
            class="flex items-center gap-2 p-4 py-1.5 rounded-lg border transition-all duration-300"
            :class="[
              step.status === 'running' ? 'bg-indigo-500/10 border-indigo-500/30 ring-2 ring-indigo-500/20 ring-offset-1' : 
          ...
          <!-- Connector -->
          <div v-if="idx < (steps?.length || 0) - 1" class="w-4 h-0.5 bg-muted mx-1"></div>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 pt-4 border-t border-border">
       <!-- CPU -->
       <div class="bg-muted/50 rounded-lg p-2.5 border border-border flex flex-col gap-1.5">
         <span class="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold truncate">{{ t('agentStatus.metrics.cpu') }}</span>
         <div class="flex items-center justify-between">
            <span class="text-sm font-mono font-semibold text-foreground">{{ cpuUsage || 0 }}%</span>
       ...
       <!-- Memory -->
       <div class="bg-muted/50 rounded-lg p-2.5 border border-border flex flex-col gap-1.5">
         <span class="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold truncate">{{ t('agentStatus.metrics.memory') }}</span>
         <div class="flex items-center justify-between">
            <span class="text-sm font-mono font-semibold text-foreground">{{ memoryUsage || 0 }}MB</span>
            <div class="w-12 h-1 bg-muted rounded-full overflow-hidden">
       ...
       <!-- Network -->
       <div class="bg-muted/50 rounded-lg p-2.5 border border-border flex flex-col gap-1.5">
         <span class="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold truncate">{{ t('agentStatus.metrics.network') }}</span>
         <div class="flex items-center gap-2">
            <span class="text-sm font-mono font-semibold text-foreground">↓{{ networkUsage || 0 }}KB/s</span>
         </div>
       </div>

       <!-- Errors -->
       <div class="bg-muted/50 rounded-lg p-2.5 border border-border flex flex-col gap-1.5">
         <span class="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold truncate">{{ t('agentStatus.metrics.errors') }}</span>
         <div class="flex items-center gap-2">
            <span 
              class="text-sm font-mono font-semibold transition-colors duration-300" 
              :class="errorCount && errorCount > 0 ? 'text-red-600' : 'text-foreground'"
            >
              {{ errorCount || 0 }}
            </span>
            <span class="text-[10px] text-muted-foreground">{{ t('agentStatus.metrics.events') }}</span>
         </div>
       </div>
    </div>
