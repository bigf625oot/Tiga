...
<template>
  <div class="relative group select-none">
    ...
    <Card :class="cn('w-[140px] transition-all duration-300 dark:bg-card/95 backdrop-blur-sm overflow-visible border shadow-md hover:shadow-lg group-hover:border-primary/50', statusColorClass)">
      ...
      <div class="p-2 flex flex-col gap-1.5">
        ...
        <div class="flex items-start justify-between gap-1.5">
          <div class="flex items-center gap-1.5 min-w-0">
            <div 
              ...
              :class="iconBgClass"
            >
              <component :is="IconComponent" class="w-3.5 h-3.5" :class="iconColorClass" />
            </div>
            <div class="flex flex-col min-w-0 justify-center gap-0.5">
              <span class="text-xs font-semibold text-foreground truncate leading-none">
                {{ props.data.label }}
              </span>
              <span class="text-[9px] text-muted-foreground uppercase tracking-wider truncate leading-none">
                {{ subTypeLabel }}
              </span>
            </div>
          </div>
        ...
        <!-- Config Summary -->
        <div v-if="configSummary" class="flex items-center gap-1 px-1.5 py-0.5 mt-0.5 bg-background/50 rounded border border-border/50 text-[9px] text-muted-foreground truncate">
