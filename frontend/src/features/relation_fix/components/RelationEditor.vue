<template>
  <div class="h-full flex flex-col gap-4 p-4 overflow-y-auto">
      ...
      <div class="flex gap-2">
        <div class="relative flex-1">
          <input 
            ...
            type="text" 
            placeholder="输入节点名称..." 
            class="w-full p-4 py-2 pr-8 border rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          <button 
            v-if="searchQuery"
            @click="searchQuery = ''"
            class="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-slate-600"
          >
    ...
    <div class="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
      ...
      <div class="space-y-3">
        <div>
          <label class="block text-xs text-muted-foreground mb-1">主节点</label>
          <input v-model="detectMainNode" type="text" class="w-full p-4 py-2 border rounded-md text-sm" placeholder="例如: 中国联通" />
        </div>
        <div>
          <label class="block text-xs text-muted-foreground mb-1">包含关键词</label>
          <input v-model="detectKeyword" type="text" class="w-full p-4 py-2 border rounded-md text-sm" placeholder="例如: 联通" />
        </div>
        <button 
    ...
    <div v-if="fixes.length > 0" class="bg-white p-4 rounded-lg shadow-sm border border-slate-200 flex-1 min-h-[200px] flex flex-col">
      ...
      <div class="flex-1 overflow-y-auto space-y-2 text-sm">
        <div v-for="(fix, idx) in fixes" :key="idx" class="flex items-start gap-2 p-2 bg-muted/50 rounded border border-border">
          <input type="checkbox" :value="fix" v-model="selectedFixes" class="mt-1" />
          <div>
            <div class="font-medium">{{ fix.source }} <span class="text-muted-foreground">-></span> {{ fix.target }}</div>
            <div class="text-xs text-muted-foreground">{{ fix.reason }}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
      <div class="flex border-b border-slate-200 mb-4">
        <button 
          ...
          :class="activeTab === 'create' ? 'text-primary' : 'text-muted-foreground hover:text-slate-700'"
        >
          新建关系
          <div v-if="activeTab === 'create'" class="absolute bottom-0 left-0 w-full h-0.5 bg-primary"></div>
        </button>
        <button 
          @click="activeTab = 'delete'"
          class="flex-1 pb-2 text-sm font-medium transition-colors relative"
          :class="activeTab === 'delete' ? 'text-red-600' : 'text-muted-foreground hover:text-slate-700'"
        >
          解除关系
      ...
      <div v-if="activeTab === 'create'" class="space-y-3">
        <div class="grid grid-cols-2 gap-2">
          <input v-model="newRel.source" placeholder="源节点 (例如: 北京分公司)" class="p-4 py-2 border rounded-md text-sm" />
          <input v-model="newRel.target" placeholder="目标节点 (例如: 中国联通)" class="p-4 py-2 border rounded-md text-sm" />
        </div>
        <div>
           <label class="block text-xs text-muted-foreground mb-1 ml-1">关系类型</label>
           <input v-model="newRel.type" placeholder="例如: 属于、包含、位于" class="w-full p-4 py-2 border rounded-md text-sm" />
        </div>
        <button 
      ...
      <div v-else class="space-y-3">
        <div v-if="!currentRelations || currentRelations.length === 0" class="text-xs text-muted-foreground text-center py-4 bg-muted/50 rounded">
          当前无可见关系，请先搜索并加载节点
        </div>
        <div v-else>
          ...
          <div class="flex flex-col gap-2 max-h-[240px] overflow-y-auto pr-1 custom-scrollbar">
            <div 
              ...
              :class="selectedRelationsToDelete.includes(rel) ? 'bg-red-50 border-red-100' : 'bg-white border-border hover:border-slate-300'"
            ...
            >
              ...
              <div class="text-xs flex-1 break-all">
                ...
                <div class="mt-1 text-[10px] text-muted-foreground" v-if="rel.label">类型: {{ rel.label }}</div>
              </div>
            </div>
          </div>
          <button 
            ...
            class="w-full m-4 py-2 bg-red-50 text-red-600 border border-red-200 rounded-md text-sm hover:bg-red-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          >
    ...
    <div class="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
      ...
      <div class="flex gap-2">
        <button @click="$emit('backup')" class="flex-1 py-2 border border-slate-300 rounded-md text-sm hover:bg-muted/50">备份图谱</button>
