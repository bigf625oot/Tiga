<template>
  <div 
    class="rounded-xl border p-5 relative overflow-hidden group transition-all duration-300 flex flex-col gap-5"
    :class="isLightMode ? 'bg-white border-slate-200 hover:border-blue-500/30 shadow-[0_2px_8px_rgba(0,0,0,0.04)]' : 'bg-[#111827] border-gray-800 hover:border-blue-500/50'"
  >
    <!-- Header -->
    ...
    <div class="grid grid-cols-2 gap-3">
      <div 
        ...
        :key="idx" 
        class="rounded-xl p-3 flex flex-col justify-between h-24"
        :class="isLightMode ? 'bg-slate-50' : 'bg-[#1F2937]/50'"
      >
        <div class="flex justify-between items-start">
            <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">{{ metric.label }}</span>
        ...
        <div class="flex items-end justify-between mt-2">
          <span class="text-[20px] font-inter font-bold tracking-tight leading-none" :class="isLightMode ? 'text-slate-800' : 'text-white'">{{ metric.value }}</span>
        </div>
      </div>
    </div>
    ...
    <div 
      class="rounded-xl p-3 flex items-center gap-4 transition-colors"
      :class="isLightMode ? 'bg-slate-50' : 'bg-[#1F2937]/50'"
    >
      <div class="relative w-10 h-10 flex items-center justify-center">
         <svg class="w-full h-full -rotate-90" viewBox="0 0 36 36">
            <path class="text-slate-200 dark:text-gray-700" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" stroke-width="3" />
         ...
         <span class="absolute text-[10px] font-bold" :class="isLightMode ? 'text-slate-600' : 'text-gray-300'">{{ data.health_score }}</span>
      </div>
      <div class="flex flex-col">
         <span class="text-[11px] font-bold uppercase tracking-wider text-slate-400">系统健康度</span>
         <span class="text-[13px] font-medium" :class="isLightMode ? 'text-slate-700' : 'text-gray-300'">运行平稳，无异常告警</span>
      </div>
    </div>
  </div>
</template>
