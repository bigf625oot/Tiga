<template>
  <div 
    class="absolute bottom-0 left-0 right-0 border-t backdrop-blur-sm z-10 transition-all duration-300"
    :class="isLightMode ? 'bg-white/90 border-border' : 'bg-[#0B0C10]/90 border-gray-800'"
  >
    <div 
      class="flex items-center justify-between px-4 py-2 border-b transition-colors"
      :class="isLightMode ? 'bg-gray-50 border-border' : 'bg-[#111827] border-gray-800'"
    >
      <div class="flex items-center gap-2">
        <svg class="w-4 h-4 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z" />
        </svg>
        <span class="text-[15px] font-semibold" :class="isLightMode ? 'text-gray-900' : 'text-white'">实时日志流</span>
      ...
      <button 
        ...
        :class="isLightMode ? 'text-gray-500 hover:text-gray-900 bg-gray-200 hover:bg-gray-300' : 'text-gray-400 hover:text-white bg-gray-800 hover:bg-gray-700'"
      >
        暂停
      </button>
    </div>
    <div class="h-40 overflow-hidden relative font-mono text-sm">
      <div class="absolute inset-0 p-4 space-y-2 animate-scroll">
        <div 
          ...
          :class="isLightMode ? 'bg-gray-50/50 border-border hover:bg-gray-100' : 'bg-[#1F2937]/50 border-gray-800 hover:bg-[#1F2937]'"
        >
          <span class="text-gray-500 shrink-0 font-din text-[13px]">{{ log.timestamp }}</span>
          <div class="shrink-0">
          ...
          <span class="truncate" :class="{
            ...
            'text-primary': isLightMode && log.level === 'info',
            ...
            'text-gray-700': isLightMode && !['success', 'info', 'warning', 'error'].includes(log.level)
          }">{{ log.message }}</span>
        </div>
