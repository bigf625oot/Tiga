...
<template>
  <div v-if="sources && sources.length > 0" class="source-panel mt-4 border-t border-border p-4">
    <!-- Header / Toggle -->
    <div 
      class="flex items-center justify-between cursor-pointer py-2 hover:bg-muted/50 rounded px-2 transition-colors select-none"
    ...
    >
      ...
      <div class="text-muted-foreground transform transition-transform duration-200" :class="{ 'rotate-180': isExpanded }">
    ...
    >
      ...
      <template v-if="isDocMode">
        <div 
          ...
          :key="item.docId || idx"
          class="doc-card p-4 rounded-lg border border-slate-200 bg-white hover:shadow-md hover:border-indigo-200 transition-all cursor-pointer group relative"
          @click="handleDocClick(item)"
        >
          <div class="flex items-start justify-between gap-2">
            <div class="flex items-center gap-2 min-w-0">
                <div class="w-8 h-8 rounded bg-primary/10 flex items-center justify-center shrink-0 text-blue-500">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div v-if="activeDocId === item.docId" class="mt-2 pt-2 border-t border-border text-xs text-slate-600 leading-relaxed bg-muted/50/50 p-2 rounded">
             {{ item.summary || '暂无摘要' }}
          </div>
        </div>
      </template>
      ...
      <template v-else>
        <div 
          ...
          class="chunk-card p-4 rounded-lg border border-slate-200 bg-white hover:shadow-md hover:border-indigo-300 transition-all cursor-pointer group relative"
        ...
        >
          <div class="flex justify-between items-start mb-1">
            <div class="flex items-center gap-1.5">
                <span class="w-5 h-5 flex items-center justify-center bg-indigo-100 text-indigo-600 text-[10px] font-semibold rounded">
                    #{{ item.chunkId || idx + 1 }}
                </span>
                <span v-if="item.pageNo" class="text-[10px] text-muted-foreground bg-muted px-1.5 rounded">
                    P{{ item.pageNo }}
                </span>
            </div>
          ...
          >
            <span class="bg-white text-indigo-600 text-xs font-medium p-4 py-1.5 rounded-full shadow-sm border border-indigo-100 flex items-center gap-1">
...
<script setup>
/**
 * @场景    在回答后展示知识来源，支持文档摘要查看与片段定位
 * @功能    提供文档模式/片段模式切换渲染，并发出 locate-node 与 show-doc-summary 事件
 * @依赖    Vue3、dayjs
 * @备注    当前主要由 MessageItem 调用，若消息组件收敛需同步迁移该面板入口
 */
