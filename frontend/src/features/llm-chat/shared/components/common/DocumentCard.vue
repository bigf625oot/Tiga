<template>
  <div
    class="inline-flex items-center gap-3 px-3 py-2 my-1 rounded-lg border bg-card text-card-foreground shadow-sm hover:shadow hover:border-primary/40 hover:bg-muted/40 cursor-pointer transition-all duration-200 group align-middle max-w-[320px] w-full"
    @click="handleClick"
    title="点击预览文档"
  >
    <!-- PDF Icon -->
    ...
    <div class="flex flex-col flex-1 min-w-0">
      <span class="text-sm font-medium text-foreground truncate w-full" :title="displayTitle">
        {{ displayTitle }}
      </span>
      <div class="flex items-center gap-2 mt-0.5">
        <span class="text-[11px] text-muted-foreground font-mono">doc#{{ docId }}</span>
        <span v-if="createdAt" class="text-[11px] text-muted-foreground truncate">· {{ createdAt }}</span>
