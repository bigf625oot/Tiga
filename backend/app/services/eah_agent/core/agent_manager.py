            ...
            if isinstance(tools_config, list):
                for tool_entry in tools_config:
                    if isinstance(tool_entry, str):
                        ...
                        # Handle configured tools (e.g. {name: 'openbb', config: {api_key: '...'}})
                        tool_name = tool_entry.get("name", "").lower()
                        # Skip skills, they are handled later
                        if tool_entry.get("type") == "skill":
                            continue
                        ...
                        if tool_name in available_tools:
                            try:
                                ...
                                loaded_dynamic_tools.add(tool_name)
                                logger.info(f"Loaded dynamic tool: {tool_name} with config")
                            except Exception as e:
                                logger.error(f"Failed to load dynamic tool {tool_name} with config: {e}")
...
"""
            ...
            if is_sandbox_enabled:
                try:
                    from app.services.eah_agent.tools.libs.sandbox_tools import SandboxTools
...
"""
            ...
            # Check for show_tool_calls in model_config
