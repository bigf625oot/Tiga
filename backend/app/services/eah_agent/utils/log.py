"""
该模块用于 eah_agent 服务的日志体系封装与统一配置：
- 基于 RichHandler 提供更友好的终端输出，并按来源（agent/team/workflow）区分展示样式与颜色。
- 提供自定义 Logger（支持居中标题输出）以及构建/切换默认日志实例的能力。
- 提供便捷的 log_* 方法与全局 debug 开关，便于在不同运行模式下统一控制日志等级与输出行为。
