            ...
            for node_id, data in nodes.items():
                ...
                session.execute_write(
                    lambda tx: tx.run(
                        ...
                        name=name, doc_id=doc_id, type=ntype, attrs=attrs
                    )
                )
                if sc and isinstance(sc, list):
                    for item in sc:
                        ...
                        session.execute_write(
                            lambda tx: tx.run(
                                ...
                                "SET r.preview=$prev",
                                doc_id=doc_id, name=name, cid=cid, prev=prev
                            )
                        )
            for edge_id, ed in edges.items():
                ...
                session.execute_write(
                    lambda tx: tx.run(
                        ...
                        src=src, tgt=tgt, label=label, doc_id=doc_id
                    )
                )
    except Exception as e:
