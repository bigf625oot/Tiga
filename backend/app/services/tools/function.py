        ...
        try:
            ...
            if self.function.tool_hooks is not None:
                ...
                if self.arguments is None or self.arguments == {}:
                    ...
                    args = {**entrypoint_args, **self.arguments}

                # Handle both sync and async entrypoints
                if isasyncgenfunction(self.function.entrypoint):
                    self.result = self.function.entrypoint(**args)  # Store async generator directly
                elif iscoroutinefunction(self.function.entrypoint):
                    self.result = await self.function.entrypoint(**args)  # Await coroutine result
                elif isgeneratorfunction(self.function.entrypoint):
                    self.result = self.function.entrypoint(**args)  # Store sync generator directly
                else:
                    # Sync function - wrap in threadpool to avoid blocking
                    self.result = await run_in_threadpool(self.function.entrypoint, **args)
