...
from app.schemas.search_agent.news_schemas import (
    CustomNewsSearchRequest,
    NewsItem,
    NewsSearchRequest,
    NewsSearchResponse,
)
from app.services.intelligence.search.search_service import perform_search, perform_tavily_search
...
async def _search_gov_site_tier1(
    session: aiohttp.ClientSession, site_url: str, keyword: str, target_date: Optional[str] = None
) -> List[NewsItem]:
    """
    ...
    try:
        ...
        logger.error(f"Tier 1 search failed for {site_url}: {e}")

    return results


def _search_authoritative_tier2_sync(
    keyword: str, domains: List[str], max_results: int, target_date: Optional[str] = None
) -> List[NewsItem]:
    """
    ...
    tavily_results = perform_tavily_search(
        query=search_query, search_depth="advanced", include_domains=domains, max_results=max_results
    )
    ...
    for res in tavily_results:
        ...
        results.append(
            NewsItem(
                ...
                news_time=pub_date
                if pub_date
                else (target_date if target_date else datetime.now().strftime("%Y-%m-%d")),
                url=res.get("url"),
                title=res.get("title"),
...
async def search_news(request: NewsSearchRequest) -> NewsSearchResponse:
    ...
    """
    logger.debug(f"[DEBUG] search_news called with keywords: {request.keywords}")
    all_results = []

    # 1. Tier 1: Core Gov Sites
    if request.gov_sites:
        logger.debug(f"[DEBUG] Starting Tier 1 search on: {request.gov_sites}")
    ...
    if request.authoritative_sites:
        logger.debug("[DEBUG] Starting Tier 2 (Tavily) search")
        ...
        for keyword in request.keywords:
            ...
            logger.debug(f"[DEBUG] Tier 2 result count for '{keyword}': {len(res)}")
            all_results.extend(res)

    # 3. Tier 3: Global Search
    logger.debug("[DEBUG] Starting Tier 3 (Global/Alibaba) search")
    for keyword in request.keywords:
        res = await _search_global_tier3(keyword, request.max_results, request.target_date)
        logger.debug(f"[DEBUG] Tier 3 result count for '{keyword}': {len(res)}")
        all_results.extend(res)

    logger.debug(f"[DEBUG] Total raw results: {len(all_results)}")
...
class NewsQueryExecutor:
    ...
    def __init__(
        ...
        enabled_tiers: List[str] = ["crawler", "tavily", "aliyun"],
    ):
    ...
    async def extract_title_and_date(self, content: str, url: str = None):
        ...
        if match:
            ...
            date_str = (
                date_str.replace("年", "-").replace("月", "-").replace("日", "").replace("/", "-").replace(".", "-")
            )
            news_time = date_str
    ...
    async def execute(self) -> List[NewsItem]:
        ...
        for keyword in self.keywords:
            ...
            if "aliyun" in self.enabled_tiers:
                ...
                logger.debug(f"[DEBUG] Executing custom query (Aliyun): {query}")
                ...
                if response.success:
                    try:
                        ...
                        logger.error(f"Error parsing Aliyun results for {keyword}: {e}")
            ...
            if "tavily" in self.enabled_tiers:
                try:
                    ...
                    logger.debug(f"[DEBUG] Tavily custom search found {len(tavily_res)} results for query: {query}")
                    ...
                    logger.error(f"Error executing Tavily custom search: {e}")
                ...
                try:
                    ...
                    async with aiohttp.ClientSession() as session:
                        for site in default_gov_sites:
                            ...
                            crawler_res = await _search_gov_site_tier1(
                                session, site, keyword, self.time_range
                            )  # time_range might act as target_date filter
                            logger.debug(f"[DEBUG] Crawler custom search found {len(crawler_res)} results for {site}")
                    ...
                    logger.error(f"Error executing Crawler custom search: {e}")
            ...
            try:
                ...
                if response.success:
                    try:
                            ...
                            if item.get("title"):
                                ...
                                if raw_title and raw_title not in ["新闻", "首页", "Home", "News", "无标题", "Unknown"]:
                                    ...
                                    if content:
                                        ...
                                        title = (
                                            first_sentence[:30] + "..." if len(first_sentence) > 30 else first_sentence
                                        )
                        ...
                        logger.error(f"Error parsing results for {keyword}: {e}")
            except Exception as e:
...
async def execute_custom_news_search(request: CustomNewsSearchRequest) -> NewsSearchResponse:
    logger.debug(f"[DEBUG] custom_search called with keywords: {request.keywords}, group: {request.group_tag}")
    executor = NewsQueryExecutor(
        keywords=request.keywords,
        group_tag=request.group_tag,
        keyword_constraints=request.keyword_constraints,
    ...
    logger.debug(f"[DEBUG] custom_search raw result count: {len(results)}")
    ...
    return NewsSearchResponse(
        success=True,
        data={"results": [item.dict() for item in results], "count": len(results)},
