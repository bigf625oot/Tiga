...
from app.services.intelligence.knowledge.rag.schemas import (
    AugmentRequest,
    AugmentResponse,
    EmbedRequest,
    EmbedResponse,
...
from app.services.intelligence.knowledge.rag.retrieval.providers import LightRAGVectorStore, OpenAICompatLLM, OpenAIEmbedder


class RagService:
    @staticmethod
    async def _get_active_llm_model(db: AsyncSession) -> Optional[LLMModel]:
        """获取当前激活的 LLM 模型（非 embedding 类型）"""
    ...
    @staticmethod
    async def _get_active_embedding_model(db: AsyncSession) -> Optional[LLMModel]:
    ...
    async def embed_documents(self, req: EmbedRequest, db: AsyncSession) -> EmbedResponse:
        ...
        embed_model = await self._get_active_embedding_model(db)
        if not embed_model:
            return EmbedResponse(vectors=[])
        embedder = OpenAIEmbedder(
            api_key=embed_model.api_key, base_url=embed_model.base_url, model=embed_model.model_id
        )
    ...
    async def augment_prompt(self, req: AugmentRequest, db: AsyncSession) -> AugmentResponse:
        ...
        llm = await self._get_active_llm_model(db)
        if not llm:
        ...
        messages.append(
            {
                "role": "system",
                "content": "你是一个中文知识库助手，仅基于提供的证据回答，若证据不足请明确说明，不要编造。",
            }
        )
        if req.history:
        ...
        sources = [
            {"source": "vector", "content": r.get("preview") or "", "score": float(r.get("score") or 0.0)} for r in refs
        ][:5]
    ...
    async def health_check(self, db: AsyncSession) -> HealthResponse:
        ...
        embed_model = None
        try:
            llm_model = await self._get_active_llm_model(db)
            embed_model = await self._get_active_embedding_model(db)
        except Exception:
            pass
        return HealthResponse(
            status="ok",
            llm_model=getattr(llm_model, "model_id", None),
            embed_model=getattr(embed_model, "model_id", None),
        )
