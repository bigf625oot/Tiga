...
from app.services.platform.utils.markdown import to_markdown
from app.services.intelligence.knowledge.rag.utils.chunking import chunk_text

logger = logging.getLogger(__name__)


def _clean_answer_text(answer: Any, collapse_newlines: bool = False) -> str:
    """
    清理 LLM 回答文本：统一引用格式、移除尾部引用列表、清理空行。

    Args:
        answer: 原始回答（任意类型，会转为字符串）
        collapse_newlines: 是否将连续3个以上换行符压缩为2个
...
class QAService:
    ...
    async def qa_stream(
        ...
        try:
            ...
            answer_text = _clean_answer_text(answer)
    ...
    async def qa(
        ...
        try:
            ...
            answer_text = _clean_answer_text(answer, collapse_newlines=True)
