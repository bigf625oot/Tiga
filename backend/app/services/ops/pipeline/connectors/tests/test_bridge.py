...
import unittest
from app.services.ops.pipeline.connectors.bridge import DataSourceBridge
...
class TestDataSourceBridge(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.session_patcher = patch('app.services.ops.pipeline.connectors.bridge.SessionLocal')
        self.MockSession = self.session_patcher.start()
        self.mock_db = self.MockSession.return_value
        
        # Patch decrypt_field
        self.decrypt_patcher = patch('app.services.ops.pipeline.connectors.bridge.decrypt_field')
        self.mock_decrypt = self.decrypt_patcher.start()
        self.mock_decrypt.side_effect = lambda x: f"decrypted_{x}"
        
        # Patch settings
        self.settings_patcher = patch('app.services.ops.pipeline.connectors.bridge.settings')
