...
try:
    ...
    logger.warning(f"Alibaba Cloud SDK not available: {e}")
...
async def perform_search(
    query: str, service_name: str = "default", app_name: str = "ops-web-search-001"
) -> SearchResponse:
    """
    执行网络搜索的内部函数 (Aliyun)
    """
    if not ALIBABA_SDK_AVAILABLE:
        return SearchResponse(
            success=False, message="Alibaba Cloud SDK not available. Please install required dependencies."
        )

    if not settings.ALIBABA_CLOUD_BEARER_TOKEN:
        return SearchResponse(success=False, message="Alibaba Cloud Bearer Token not configured.")
    ...
    try:
        ...
        # 配置客户端
        config = Config(bearer_token=api_key, endpoint=endpoint, protocol=protocol)
        ...
        runtime = RuntimeOptions(read_timeout=30000, connect_timeout=10000)
        headers = {}
        ...
        return SearchResponse(success=False, message=f"Search failed: {str(e)}")


def perform_tavily_search(
    query: str, search_depth: str = "advanced", include_domains: Optional[List[str]] = None, max_results: int = 10
) -> List[Dict[str, Any]]:
    """
    使用Tavily API执行高级搜索
    """
    ...
    try:
        ...
        response = tavily.search(
            query=query, search_depth=search_depth, include_domains=include_domains, max_results=max_results
        )

        raw_results = response.get("results", [])
        logger.debug(f"Tavily found {len(raw_results)} raw results")
