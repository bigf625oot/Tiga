...
class OpenClawHttpClient:
    ...
    def __init__(self, base_url: Optional[str] = None, token: Optional[str] = None, timeout: float = 20.0):
        ...
        # Security: SSL Verification configuration
        # Default to True for production safety, allow disabling via config for dev/test
        self.verify_ssl = getattr(settings, "OPENCLAW_VERIFY_SSL", True)
    ...
    async def chat_completion(self, messages: list, model: str = "openclaw:main", stream: bool = True) -> Dict[str, Any]:
        ...
        logger.info(f"发起 OpenAI 兼容对话请求: url={url}, model={model}")
        payload = {
            "model": model,
            "messages": messages,
            "stream": stream
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                logger.error(f"OpenAI 对话请求失败: status={response.status_code}, body={error_text}")
                return {"error": f"HTTP {response.status_code}: {error_text}"}
                
            return response.json()
        except Exception as e:
            logger.error(f"OpenAI 对话请求异常: {e}")
            return {"error": str(e)}