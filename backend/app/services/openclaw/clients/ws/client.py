...
from app.services.openclaw.node.auth.node_auth_service import DeviceIdentityManager
...
class OpenClawWsClient:
    ...
    def __init__(
        ...
        *,
        ...
        raw_token = settings.OPENCLAW_GATEWAY_TOKEN or settings.OPENCLAW_TOKEN
        self._token: str | None = raw_token.strip() if raw_token else None
    ...
    async def _handshake(self) -> None:
        ...
        if not res.get("ok"):
            ...
            # Check both code and message for token/auth issues
            is_auth_error = (
                "TOKEN" in code_str or 
                "AUTH" in code_str or 
                "SIGNATURE" in code_str or 
                "SCOPE" in code_str or
                "unauthorized" in msg_str or
                "token mismatch" in msg_str or
                "invalid token" in msg_str
            )
            
            if is_auth_error:
                logger.warning(f"Device Token 可能失效、签名错误或权限不足 ({code}: {msg})，清除缓存以重试")
                try:
