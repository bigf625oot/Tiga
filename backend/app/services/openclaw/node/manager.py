...
class NodeManager:
    ...
    def __init__(self):
        ...
        if settings.OPENCLAW_BASE_URL:
            # We delay import or use lazy loading to avoid circular dependency if possible
            # But since we moved files, OpenClawService is now in gateway/gateway.service.py
            # which might depend on node_manager? Let's check.
    ...
    async def dispatch_command(
        ...
        if not candidates:
            return {"total": 0, "sent": 0, "message": "No nodes found"}
