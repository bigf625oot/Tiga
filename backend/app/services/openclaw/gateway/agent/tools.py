...
from app.services.openclaw.clients.ws.client import OpenClawWsClient  # 仅用于类型提示或特殊WS调用，主要逻辑已迁移
    ...
    def _fallback_invoke(self, tool_name: str, params: Dict[str, Any]) -> str:
        if tool_name == "web_search" and BRAVE_AVAILABLE:
            try:
                ...
                log_error(f"Brave fallback failed: {e}")
        ...
        if tool_name == "web_fetch" and TRAFILATURA_AVAILABLE:
            try:
                ...
                log_error(f"Trafilatura fallback failed: {e}")
