...
async def query_data(request: VannaRequest):
    ...
    [Updated] Uses IntentClassifier to route between SQL and KG engines.
    """
    ...
    # Determine message type for frontend ChatCard
    msg_type = "intelligent_query" if intent in [QueryIntent.SQL_QUERY, QueryIntent.STRUCTURED_QUERY] else "knowledge_qa"
    
    # Wrapper to inject type meta event and format as SSE
    async def response_wrapper(generator):
        ...
        async for chunk in generator:
            try:
                # Backend yields JSON lines: {"step":..., "content":..., "type":...}
                obj = json.loads(chunk)
                content = obj.get("content")
                b_type = obj.get("type")
                
                # Map backend type to SSE event
                evt = "text"
                if b_type == "process":
                    evt = "think"
                elif b_type == "error":
    ...
    # KG_QUERY is now handled inside data_query_service.query via KGQueryService
    return StreamingResponse(
        response_wrapper(data_query_service.query(request.question, session_id=request.session_id, intent=intent)),
