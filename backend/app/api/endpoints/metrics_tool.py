...
from app.services.intelligence.knowledge.extractor.document_parser import parse_document
from app.services.domain.chatbi.extraction import run_extraction, generate_prompt
...
async def parse_knowledge_doc_endpoint(doc_id: int, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if doc.oss_key and storage_service.provider:
...
async def batch_generate_prompts(request: BatchPromptRequest):
    ...
    for ind in request.indicators:
        ...
        results.append({"indicator_name": ind.get("name"), "prompt": prompt})
    return results
...
async def extract_metric(request: ExtractionRequest):
    ...
    # 2. Run LLM
    result = await run_extraction(prompt=prompt, text_content=request.text_content, model=request.model)

    return ExtractionResponse(status=result["status"], content=result["content"], prompt_used=prompt)
