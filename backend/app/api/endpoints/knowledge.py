...
class QARequest(BaseModel):
    ...
    mode: str = "auto"  # "auto", "rag", "kg", "data"
    ...
    return [
        {
            ...
            "sql_query": (h.meta_data or {}).get("sql_query") if getattr(h, "meta_data", None) else None,
            "chart_config": (h.meta_data or {}).get("chart_config") if getattr(h, "meta_data", None) else None,
            "data_content": (h.meta_data or {}).get("data_content") if getattr(h, "meta_data", None) else None,
        }
        for h in history
    ]
...
async def document_qa(doc_id: int, request: QARequest, req: Request, db: AsyncSession = Depends(get_db)):
    ...
    logger.info(
        f"[问答][{tid}] POST 文档 {doc_id} 范围={request.scope} 模式={request.mode} 会话={request.session_id}"
    )
...
async def global_qa(request: QARequest, req: Request, db: AsyncSession = Depends(get_db)):
    tid = req.headers.get("X-Trace-Id") or str(uuid.uuid4())
    logger.info(
        f"[问答][{tid}] POST 全局 QA 范围={request.scope} 模式={request.mode} 会话={request.session_id}"
    )
    ...
    return StreamingResponse(
        qa_service.qa_stream(
            0, request.query, db, history=request.history, trace_id=tid, scope=scope, mode=request.mode, session_id=request.session_id
        ),
        media_type="text/event-stream",
    )
