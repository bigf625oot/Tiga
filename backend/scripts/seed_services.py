...
async def seed():
    async with AsyncSessionLocal() as db:
        ...
        logger.info("Discovering and seeding Python Tools...")
        discovered_tools = discover_tools()
        
        for tool_name, tool_class in discovered_tools.items():
            # tool_name is module name, e.g. "calculator"
            ...
            result = await db.execute(select(Skill).where(Skill.name == tool_name))
            existing = result.scalars().first()
            ...
            meta_data = {
                "type": "tool",
                "module": f"app.services.tools.{tool_name}",
                "class": tool_class.__name__
            }

            if not existing:
                logger.info(f"Adding Tool Skill: {tool_name} (Active: {is_available})")
                skill = Skill(
                    id=str(uuid.uuid4()),
                    name=tool_name,
                    description=description,
                    content=None, # It's a python tool, not a prompt skill
                    category=category,
                    author="System",
                    version="1.0.0",
                    is_official=True,
                    is_active=is_available, # Only activate if available
                    meta_data=meta_data
                )
                db.add(skill)
            else:
                logger.info(f"Updating existing Tool Skill: {tool_name}")
                existing.description = description
                existing.meta_data = meta_data
                existing.category = category
                existing.is_active = is_available
                db.add(existing)
