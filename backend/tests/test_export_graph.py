...
import yaml
from scripts.export_graph_data import GraphExporter
...
class TestGraphExporter(unittest.TestCase):
    def setUp(self):
        ...
        self.config_data = {
            "database": {"type": "sqlite", "database": str(self.db_path)},
            "output": {"output_dir": str(self.test_dir), "update_mode": "overwrite"},
            "processing": {"chunk_size": 10},
            "graph": {
                "entities": [
                    {"table": "users", "id_column": "id", "label": "User", "attributes": ["name"]},
                    {"table": "orders", "id_column": "id", "label": "Order", "attributes": ["amount"]},
                ],
                "relationships": [
                    {
                        "source_table": "orders",
