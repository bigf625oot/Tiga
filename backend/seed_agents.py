...
async def seed():
    async with AsyncSessionLocal() as db:
        ...
        for data in AGENTS_DATA:
            # Check if exists
            # For simplicity, we just create new ones or check by name?
            # Ideally we check by name to avoid duplicates on re-run
            # But async query here is tricky without imports.
            # Let's just try to insert.

            # Since we don't have easy lookup here without pulling in CRUD properly,
